...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None: # place.ant is original
            place.ant = self  # self is new ant
        else:
            # BEGIN Problem 8b
            if place.ant.can_contain(self) is True and place.ant.is_container: # check if original ant can contain new ant
                place.ant.store_ant(self)
            elif self.can_contain(place.ant) is True and self.is_container: # check if new ant can contain original ant
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        count = 0
        b = self.place
        while b.is_hive == False:
            if count > self.upper_bound:
                return None
            if random_bee(b.bees) != None and self.lower_bound <= count <= self.upper_bound: 
                return random_bee(b.bees)
            b = b.entrance
            count += 1 
        return None
        # END Problem 3 and 4
...
class ShortThrower(ThrowerAnt):
    ...
    def nearest_bee(self):
        ...
        while target_place is not None:
            if not target_place.is_hive and target_place.bees:
...
class LongThrower(ThrowerAnt):
    ...
    def nearest_bee(self):
        ...
        target_place = self.place
        while target_place is not None:
            if not target_place.is_hive and self.lower_bound <= target_place.bees < self.upper_bound:
                return random_bee(target_place.bees)
            target_place = target_place.entrance
        return None
        # END Problem 4
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        """
        # BEGIN Problem 5
        reflected_damage = amount + (self.health <= 0) * self.damage
...
class ContainerAnt(Ant):
    ...
    def store_ant(self, ant):
        # BEGIN Problem 8a
        # make sure instance attribute (ant_contained) stores the ant that was passed
    ...
    def action(self, gamestate):
        # BEGIN Problem 8a
        # check weather there is ant within the container ant
            # make sure to call the action method
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate):
        # Attack all bees in the same place
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        while current.exit is not None:
            if current.ant:
                if current.ant.is_container and current.ant.ant_contained and not current.ant.ant_contained.doubled:
                    current.ant.double()
